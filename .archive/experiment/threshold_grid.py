...
from experiment.data_collection import get_option_price, next_nth_friday
from experiment.xgboost_train import build_features, FEATURE_COLS
...
# ─────────────────────────────────────────────────────────────
# SECTION 4b: Block bootstrap robust Sharpe
# ─────────────────────────────────────────────────────────────
...
def _block_bootstrap_sharpe(
    ...
    """
    Block bootstrap the daily return series and compute Sharpe for each
    resampled path.  Blocks of `block_size` consecutive days are drawn
    with replacement to preserve short-range autocorrelation.

    Returns an array of `n_bootstrap` Sharpe ratios.
    """
    n = len(daily_returns)
    if n < block_size:
    ...
    for b in range(n_bootstrap):
        # draw n_blocks random block start indices
        ...
        ann_ret = (1 + resampled.sum()) ** (252 / n) - 1
        ann_vol = resampled.std() * np.sqrt(252)
        sharpes[b] = ann_ret / ann_vol if ann_vol > 1e-8 else 0.0

    return sharpes


def robust_sharpe(nav_df: pd.DataFrame,
                  block_size: int = 21,
                  n_bootstrap: int = 200,
                  percentile: float = 5.0) -> float:
    """
    Robust Sharpe = 5th-percentile of the block-bootstrap Sharpe distribution.

    A (tc, ts) whose Sharpe depends on a handful of lucky trades will have
    high variance across bootstrap samples → low 5th-percentile.
    A genuinely robust (tc, ts) will show stable Sharpe across samples.
    """
    r = nav_df["daily_return"].values
    sharpes = _block_bootstrap_sharpe(r, block_size=block_size,
                                      n_bootstrap=n_bootstrap)
    return float(np.percentile(sharpes, percentile))


def run_threshold_grid_robust(
    data_val: pd.DataFrame,
    skew_fn: interp1d,
    tc_grid: np.ndarray,
    ts_grid: np.ndarray,
    ...
    """
    Grid search using block-bootstrap robust Sharpe (5th percentile)
    as the objective instead of point-estimate Sharpe.
    """
    records = []
    with tqdm(total=len(tc_grid) * len(ts_grid),
              desc="  Robust grid search") as pbar:
        for tc in tc_grid:
            for ts in ts_grid:
                nav_df, metrics = run_backtest(data_val, tc, ts, skew_fn)
                rs = robust_sharpe(nav_df, block_size=block_size,
                                   n_bootstrap=n_bootstrap)
                records.append({
                    "tc": tc, "ts": ts,
                    **metrics,
                    "robust_sharpe": round(rs, 4),
                })
                pbar.update(1)
    return pd.DataFrame(records)
...
if __name__ == "__main__":
    print("=" * 60)
    print("PRISM — Threshold Grid Search (with Block Bootstrap)")
    print("=" * 60)
    ...
    print("\n[3/5] Running ROBUST grid search (block bootstrap, 200 resamples per cell)...")
    results = run_threshold_grid_robust(
        feat_val, skew_fn, tc_grid, ts_grid,
    ...
    print("\n[5/5] Best threshold pairs:")
    print("\n  --- Raw Sharpe (original method) ---")
    print(f"  tc={best_raw['tc']:.2f}  ts={best_raw['ts']:.2f}  "
          f"Sharpe={best_raw['sharpe']:.4f}  Return={best_raw['total_return']:.2%}  "
          f"MaxDD={best_raw['max_dd']:.2%}")

    print("\n  --- Robust Sharpe (block bootstrap 5th pctl) ---")
    summary = (
